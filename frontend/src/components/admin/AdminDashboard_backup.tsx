...
import { EditSignalModal } from "@/components/admin/EditSignalModal";
import { signalService, ForexSignal } from "@/services/signalService";
...
export default function AdminDashboard() {
  ...
  const [showEditSignalModal, setShowEditSignalModal] = useState(false);
  const [editingSignal, setEditingSignal] = useState<ForexSignal | null>(null);
  const [signalDeleting, setSignalDeleting] = useState<string | null>(null);
  const [signalDeleteConfirm, setSignalDeleteConfirm] = useState<{ open: boolean; signalId: string | null; pair: string | null }>({ open: false, signalId: null, pair: null });
  ...
  // Signal handlers
  const handleCreateSignal = async (signalData: Omit<ForexSignal, 'id' | 'createdAt' | 'updatedAt'>) => {
    setSavingSignal(true);
    try {
      await signalService.createSignal(signalData as any);
      // Refresh signals list
      const response = await signalService.getPublicSignals({ status: 'active' });
  ...
  const handleEditSignal = async (updatedSignal: ForexSignal) => {
    if (!editingSignal) return;
    setSavingSignal(true);
    try {
      await signalService.updateSignal(editingSignal.id, {
        ...
        accuracy: updatedSignal.accuracy,
        reliability: updatedSignal.reliability,
        timeframe: updatedSignal.timeframe,
      });
      // Refresh signals list
      const response = await signalService.getPublicSignals({ status: 'active' });
      setSignals(response.data || []);
      setShowEditSignalModal(false);
      setEditingSignal(null);
    } catch (err) {
      console.error('Error editing signal:', err);
  ...
  const handleDeleteSignal = async () => {
    if (signalDeleteConfirm.signalId) {
      setSignalDeleting(signalDeleteConfirm.signalId);
      try {
        await signalService.deleteSignal(signalDeleteConfirm.signalId);
        // Refresh signals list
        const response = await signalService.getPublicSignals({ status: 'active' });
        setSignals(response.data || []);
        setSignalDeleteConfirm({ open: false, signalId: null, pair: null });
      } catch (err) {
        console.error('Error deleting signal:', err);
        setError('Failed to delete signal');
      } finally {
        setSignalDeleting(null);
      }
    }
  };
    ...
    <div className="space-y-6 animate-fade-in transition-colors duration-300" style={{  color: colors.text.primary }}>
      ...
      {activeTab === "forex" && (
        <div className="space-y-6 px-4 sm:px-6">
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
            <Button
              className="w-full sm:w-auto bg-gradient-to-r from-blue-600 to-blue-400 text-white hover:shadow-lg transition-all"
                    ...
                    <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="grid grid-cols-3 sm:grid-cols-5 gap-2 sm:gap-3 mt-3 text-xs sm:text-sm">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="font-semibold transition-colors duration-300 truncate" style={{ color: colors.text.primary }}>{Array.isArray(signal.takeProfits) ? signal.takeProfits[0]?.toFixed(4) : signal.takeProfit?.toFixed(4) || "N/A"}</p>
                            </div>
                            <div className="min-w-0">
                              <p className="transition-colors duration-300 truncate" style={{ color: colors.text.secondary }}>Acc</p>
                              <p className="font-semibold transition-colors duration-300 truncate" style={{ color: colors.text.primary }}>{(signal.accuracy || 0).toFixed(1)}%</p>
                            </div>
                            <div className="min-w-0">
                              <p className="transition-colors duration-300 truncate" style={{ color: colors.text.secondary }}>P/L</p>
                              <p className="font-semibold transition-colors duration-300 truncate" style={{ color: (signal.profitLoss || 0) >= 0 ? '#16a34a' : '#dc2626' }}>${signal.profitLoss || 0}</p>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2 w-full sm:w-auto flex-wrap sm:flex-nowrap justify-end">
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => setSignalDeleteConfirm({ open: true, signalId: signal.id, pair: signal.pair })}
      ...
      {activeTab === "blog" && (
        <div className="space-y-6 px-4 sm:px-6">
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
            ...
            <Button className="w-full sm:w-auto bg-gradient-to-r from-blue-600 to-blue-400 text-white hover:shadow-lg transition-all">
              <Plus className="w-4 h-4 mr-1" />
      ...
      {/* Settings Tab */}
      {activeTab === "settings" && (
        <div className="space-y-6 px-4 sm:px-6 max-w-2xl mx-auto w-full">
          <div className="border rounded-lg p-4 sm:p-6 transition-colors duration-300" style={{  borderColor: colors.border }}>
            <h2 className="text-base sm:text-lg font-semibold mb-4 transition-colors duration-300" style={{ color: colors.text.primary }}>General Settings</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-xs sm:text-sm font-medium mb-1 transition-colors duration-300" style={{ color: colors.text.primary }}>Site Name</label>
                <Input placeholder="EOF Trading Platform" defaultValue="EOF Trading Platform" />
              </div>
              <div>
                <label className="block text-xs sm:text-sm font-medium mb-1 transition-colors duration-300" style={{ color: colors.text.primary }}>Site Description</label>
          ...
          <div className="border rounded-lg p-4 sm:p-6 transition-colors duration-300" style={{  borderColor: colors.border }}>
            <h2 className="text-base sm:text-lg font-semibold mb-4 transition-colors duration-300" style={{ color: colors.text.primary }}>Signal Settings</h2>
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-3">
                <label className="text-xs sm:text-sm font-medium transition-colors duration-300" style={{ color: colors.text.primary }}>Enable New Signals</label>
                <input type="checkbox" className="w-5 h-5 flex-shrink-0" defaultChecked />
              </div>
              <div className="flex items-center justify-between gap-3">
                <label className="text-xs sm:text-sm font-medium transition-colors duration-300" style={{ color: colors.text.primary }}>Email Notifications</label>
                <input type="checkbox" className="w-5 h-5 flex-shrink-0" defaultChecked />
              </div>
              <div>
                <label className="block text-xs sm:text-sm font-medium mb-1 transition-colors duration-300" style={{ color: colors.text.primary }}>Max Signals Per Day</label>
          ...
          <div className="border rounded-lg p-4 sm:p-6 transition-colors duration-300" style={{  borderColor: colors.border }}>
            <h2 className="text-base sm:text-lg font-semibold mb-4 transition-colors duration-300" style={{ color: colors.text.primary }}>User Management</h2>
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-3">
                <label className="text-xs sm:text-sm font-medium transition-colors duration-300" style={{ color: colors.text.primary }}>Allow User Registration</label>
                <input type="checkbox" className="w-5 h-5 flex-shrink-0" defaultChecked />
              </div>
              <div className="flex items-center justify-between gap-3">
                <label className="text-xs sm:text-sm font-medium transition-colors duration-300" style={{ color: colors.text.primary }}>Email Verification Required</label>
                <input type="checkbox" className="w-5 h-5 flex-shrink-0" defaultChecked />
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full">
            <Button className="flex-1 bg-gradient-to-r from-blue-600 to-blue-400 text-white hover:shadow-lg transition-all px-6">
              Save Settings
            </Button> 
            <Button className="flex-1">Reset to Defaults</Button>
          </div>
        </div>
      )}
      ...
      {/* Signal Delete Confirmation Dialog */}
      {signalDeleteConfirm.open && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4 animate-fade-in">
          <div className="rounded-lg shadow-xl w-full max-w-sm transition-colors duration-300" style={{ backgroundColor: colors.bg.card }}>
            <div className="p-4 sm:p-6 space-y-4">
              <div className="flex items-start gap-3 sm:gap-4">
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg flex items-center justify-center transition-colors duration-300 flex-shrink-0" style={{ backgroundColor: theme === 'dark' ? 'rgba(239, 68, 68, 0.2)' : 'rgb(254, 226, 226)' }}>
                  <AlertCircle className="w-5 h-5 sm:w-6 sm:h-6 text-red-600" />
                </div>
                <div className="flex-1 min-w-0">
                  <h2 className="text-base sm:text-lg font-semibold transition-colors duration-300" style={{ color: colors.text.primary }}>Delete Signal?</h2>
                  <p className="text-xs sm:text-sm mt-1 transition-colors duration-300 break-words" style={{ color: colors.text.secondary }}>
                    Are you sure you want to delete the <strong>{signalDeleteConfirm.pair}</strong> signal? This action cannot be undone.
                  </p>
                </div>
              </div>
              <div className="flex flex-col sm:flex-row gap-2 pt-4 transition-colors duration-300" style={{ borderTop: `1px solid ${colors.border}` }}>
                <Button
                  variant="outline"
                  className="flex-1 text-xs sm:text-sm"
                  onClick={() => setSignalDeleteConfirm({ open: false, signalId: null, pair: null })}
                  disabled={signalDeleting !== null}
                >
                  Cancel
                </Button>
                <Button
                  ...
                  disabled={signalDeleting !== null}
                >
                  {signalDeleting ? 'Deleting...' : 'Delete'}
                </Button>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-lg shadow-xl w-full max-w-sm transition-colors duration-300" style={{ backgroundColor: colors.bg.card }}>
            <div className="p-4 sm:p-6 space-y-4">
              <div className="flex items-start gap-3 sm:gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <h2 className="text-base sm:text-lg font-semibold transition-colors duration-300" style={{ color: colors.text.primary }}>Delete User?</h2>
                  <p className="text-xs sm:text-sm mt-1 transition-colors duration-300 break-words" style={{ color: colors.text.secondary }}>
