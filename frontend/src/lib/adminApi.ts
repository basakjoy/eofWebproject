...
export const adminApi = {
  ...
  // Articles
  getAllArticles: async (options?: { limit?: number; offset?: number }) => {
    try {
      const response = await apiClient.get('/articles', { params: options });
      return response.data;
    } catch (error) {
  ...
  updateArticle: async (
    ...
    try {
      const response = await apiClient.put(`/articles/${articleId}`, data);
      return response.data;
    } catch (error) {
