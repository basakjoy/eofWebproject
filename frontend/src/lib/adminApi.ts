...
export const adminApi = {
  ...
  updateUser: async (
    userId: string,
    data: { name?: string; role?: string; permissions?: string[]; status?: string }
  ) => {
    try {
  ...
  updateSignal: async (
    signalId: string,
    data: { status?: string; reliability?: number }
  ) => {
    try {
  ...
  // Articles
  getAllArticles: async (options?: { limit?: number; offset?: number; published?: string }) => {
    try {
      const response = await apiClient.get('/blog', { params: { ...options, published: options?.published ?? 'all' } });
      return response.data;
    } catch (error) {
  ...
  updateArticle: async (
    ...
    try {
      const response = await apiClient.put(`/blog/${articleId}`, data);
      return response.data;
    } catch (error) {
  ...
  deleteArticle: async (articleId: string) => {
    try {
      const response = await apiClient.delete(`/blog/${articleId}`);
      return response.data;
    } catch (error) {
